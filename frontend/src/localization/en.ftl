...
game-start-warning-kana = Warning: The experimental kana keyboard feature is enabled, so your score will not be submitted.
...
menu-map-owner = By: <LinkTo>{ $owner }</LinkTo>
menu-map-kpm = KPM: { $kpm }
menu-map-score = { $score } ({ $speed }x{ $mods })
menu-map-no-score = No scores
menu-map-rank = {"Rank: "}
