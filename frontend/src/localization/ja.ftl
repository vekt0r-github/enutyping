...
game-start-warning-kana = 警告：あなたは仮名入力を使っているため、プレイしてもスコアは記録されません。
...
menu-show-collections = コレクション表示
menu-hide-collections = 全マップ表示
menu-search-placeholder = ビートマップを検索
menu-map-new = ビートマップを作る
menu-map-owner = 作者：<LinkTo>{ $owner }</LinkTo>
menu-map-kpm = { $kpm } KPM
menu-map-score = { $score } ({ $speed }x{ $mods })
menu-map-no-score = スコアなし
menu-map-rank = ランク：
menu-diff-new = コレクションにビートマップを作る
menu-diff-display = { $title } // { $artist } [{ $diffname }]
menu-mapset-new = コレクションを作る
menu-mapset-delete = コレクション削除
menu-mapset-owner = 作者：<LinkTo>{ $owner }</LinkTo>
...
menu-sorting-map-date = 作成順
menu-sorting-map-length = 長さ順
menu-sorting-map-title = 曲名順
menu-sorting-map-artist = 作家名順
menu-sorting-map-owner = 作者名順
