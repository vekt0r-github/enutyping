...
error-mobile-layout = 申し訳ありませんが、{ -app-name }はデスクトップサイトだけでプレイできます。
...
navbar-play-create = <play>プレイ</play><create>エディター</create>
navbar-login = ログイン
...
settings-metadata-localization-true = 曲名と作者名をローマ字で表示する
settings-metadata-localization-false = 曲名と作者名を元言語で表示する
